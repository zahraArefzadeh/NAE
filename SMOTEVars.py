...
from Adasyn import ADASYN as MyADASYN
...
def train_and_evaluate(filename,X_train, y_train, X_test, y_test , oversampler_class, **kwargs):
    ...
    classifier = RandomForestClassifier(n_jobs=100, random_state=randomState)
...
directory_path = 'C:\\Users\\erfan\\Desktop\\BinaryDatasets'
...
for filename in os.listdir(directory_path):
    ...
    for name, method in oversampling_methods.items():
        ...
        auc_, f1_ = train_and_evaluate(filename,X_train, y_train, X_test, y_test , method, random_state=randomState)
        f1_scores[name] = f1_
        auc_scores[name] = auc_

    # Append the scores to the DataFrames
    f1_scores_df = f1_scores_df._append(f1_scores, ignore_index=True)
    auc_scores_df = auc_scores_df._append(auc_scores, ignore_index=True)

# Excel file path to write the results
excel_output_path = 'D:\\Projects\\Programming\\Learning\\Python\\Smote\\Results.xlsx'
